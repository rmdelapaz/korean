/* glossary.js — builds the course-wide vocabulary glossary from vocab-data.js.
   ...
   deduplicated reference. Each unique Korean word appears once, linked back to
   ...
   lessons use (localStorage `korean-srs`). No dependency on learn.js. */
(function () {
    'use strict';

    var VOCAB = window.KOREAN_VOCAB || {};
    var TITLES = VOCAB.titles || {};
    var SLUGS = VOCAB.slugs || {};   // lesson number -> page filename (beginner tier)
    /* Share the same spaced-repetition deck the lessons use — beginner and
       intermediate and advanced keep separate decks (matches learn.js namespacing). */
    var K_SRS = /\/advanced\//.test(location.pathname) ? 'korean-adv-srs'
              : /\/intermediate\//.test(location.pathname) ? 'korean-int-srs' : 'korean-srs';
    function lessonHref(n) { return SLUGS[n] ? SLUGS[n] + '.html' : 'korean_lesson_' + n + '.html'; }
    ...
    LESSON_KEYS.forEach(function (ln) {
        ...
        list.forEach(function (w) {
            var key = w.ko.toLowerCase();
            if (!byKey[key]) { byKey[key] = { ko: w.ko, pron: w.pron || '', en: w.en || '', lessons: [ln] }; order.push(key); }
    ...
    root.innerHTML =
        '<div class="g-controls">' +
            '<input type="search" id="g-search" class="g-search" placeholder="Search Korean, romanization or English…" aria-label="Search vocabulary" autocomplete="off">' +
            ...
            '<select id="g-sort" class="g-select" aria-label="Sort order">' +
                '<option value="tl">Sort: Korean ㄱ–ㅎ</option>' +
        ...
        '<div class="table-wrap"><table class="g-table"><thead><tr>' +
            '<th>Korean</th><th>Pronunciation</th><th>English</th><th>Lesson</th><th class="g-th-act">Review</th>' +
    ...
    function lessonLinks(lessons) {
        return lessons.slice().sort(function (a, b) { return a - b; }).map(function (n) {
            return '<a class="g-lesson-link" href="' + lessonHref(n) + '" title="' + esc(TITLES[n] || ('Lesson ' + n)) + '">L' + n + '</a>';
        }).join(' ');
    }

    function rowHTML(w) {
        var on = isTracked(w.ko);
        return '<tr data-ko="' + esc(w.ko) + '">' +
            '<td class="g-tl"><span data-speak="' + esc(w.ko) + '">' + esc(w.ko) + '</span></td>' +
    ...
    function currentFiltered() {
        ...
        var rows = WORDS.filter(function (w) {
            ...
            if (!q) return true;
            return fold(w.ko).indexOf(q) >= 0 || fold(w.en).indexOf(q) >= 0 || fold(w.pron).indexOf(q) >= 0;
        });
        ...
        rows.sort(function (a, b) {
            ...
            if (sort === 'lesson') {
                ...
                return d || a.ko.localeCompare(b.ko);
            }
    ...
    // ㄱ–ㅎ quick jump: Hangul sorts by its initial consonant, so the jump bar
    // offers the 14 base initials (tense doubles file under their plain letter).
    var INITIALS = ['ㄱ','ㄲ','ㄴ','ㄷ','ㄸ','ㄹ','ㅁ','ㅂ','ㅃ','ㅅ','ㅆ','ㅇ','ㅈ','ㅉ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ'];
    var PLAIN = { 'ㄲ': 'ㄱ', 'ㄸ': 'ㄷ', 'ㅃ': 'ㅂ', 'ㅆ': 'ㅅ', 'ㅉ': 'ㅈ' };
    function initialOf(word) {
        var code = String(word).trim().charCodeAt(0) - 0xAC00;
        if (code < 0 || code > 11171) return '';
        var c = INITIALS[Math.floor(code / 588)];
        return PLAIN[c] || c;
    }
    (function buildAZ() {
        var letters = {};
        WORDS.forEach(function (w) { var c = initialOf(w.ko); if (c) letters[c] = 1; });
        azEl.innerHTML = INITIALS.filter(function (c) { return letters[c]; }).map(function (c) {
    ...
    bodyEl.addEventListener('click', function (e) {
        ...
        if (!btn) return;
        var tl = btn.closest('tr').getAttribute('data-ko');
        var on = !isTracked(tl);
        trackWord(tl, on);
