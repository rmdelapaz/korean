/* learn.js — learning-experience layer for the Korean course.
...
(function () {
    ...
    /* This one file serves all three tiers. The beginner course lives at the
       site root; the intermediate course lives under /intermediate/ and the
       advanced course under /advanced/. Each keeps its own progress under a
       separate namespace so the three never collide. */
    var IS_INT = /\/intermediate\//.test(location.pathname);
    var IS_ADV = /\/advanced\//.test(location.pathname);
    var TIER_BASE = IS_ADV ? '/advanced/' : (IS_INT ? '/intermediate/' : '/');
    var NS = IS_ADV ? 'korean-adv-' : (IS_INT ? 'korean-int-' : 'korean-');
    var K_PROGRESS = NS + 'progress';   // { "1": {completed, visited, ts}, ... }
    var K_KNOWN    = NS + 'known';      // legacy flat map { "hola": ts, ... } — migrated into SRS
    var K_SRS      = NS + 'srs';        // { "hola": {box,due,reps}, ... }
    var K_JOURNAL  = NS + 'journal:';   // + page slug

    var VOCAB = window.KOREAN_VOCAB || {};
    var TITLES = (VOCAB.titles) || {};
    var SLUGS = (VOCAB.slugs) || {};   // { "1": "korean_greetings_essentials", ... } (beginner tier)
    var LESSON_INFO = window.KOREAN_LESSON_INFO || {};

    /* Total lessons is derived from the vocab titles, so adding a lesson to the
       data automatically updates the home-page progress meter. */
    var TOTAL_LESSONS = Object.keys(TITLES).length || 9;

    /* ---------- page identity ----------
       Beginner pages are topic-named (korean_greetings_essentials.html) and are
       mapped to a lesson number via VOCAB.slugs. Intermediate pages are numbered
       (korean_lesson_N.html). Either way we resolve a lessonNum string. */
    var page = (location.pathname.split('/').pop() || 'index').replace('.html', '') || 'index';
    var lessonNum = null;
    for (var _n in SLUGS) { if (SLUGS.hasOwnProperty(_n) && SLUGS[_n] === page) { lessonNum = _n; break; } }
    if (!lessonNum) { var lessonMatch = page.match(/korean_lesson_(\d+)/); lessonNum = lessonMatch ? lessonMatch[1] : null; }
    var isHome = (page === 'index' || page === '');

    /* Build the href for lesson N: a named beginner page if we have a slug for
       it, otherwise a numbered lesson page (relative to this tier). */
    function lessonHref(n) {
        if (SLUGS[n]) return TIER_BASE + SLUGS[n] + '.html';
        return TIER_BASE + 'korean_lesson_' + n + '.html';
    }
    ...
    function vocabImg(w, alt) {
        ...
        return '<img class="lx-word-img" src="' + src + '" alt="' + esc(alt || w.en || w.ko) +
    ...
    /* ---------- spaced-repetition store (Leitner boxes) ----------
       korean-srs: { "<es>": { box: 1..5, due: <ts>, reps: <n> } }
       ...
       occasional. The older flat korean-known map is migrated in on first read. */
    var DAY = 86400000;
    ...
    function dueFrom(words) {
        ...
        words.forEach(function (w) { var e = m[w.ko]; if (e && e.due <= now) out.push(w); });
        return out;
    }
    function trackedFrom(words) {
        var m = srsMap(), seen = {}, out = [];
        words.forEach(function (w) { if (m[w.ko] && !seen[w.ko]) { seen[w.ko] = 1; out.push(w); } });
        return out;
    }
    function flatVocab() {
        var all = [], seen = {};
        for (var ln in VOCAB) {
            ...
            VOCAB[ln].forEach(function (w) { if (!seen[w.ko]) { seen[w.ko] = 1; all.push(w); } });
        }
        return all;
    }
    ...
    /* ==========================================================
       ...
       ========================================================== */
    /* Korean has its own proficiency scale: TOPIK levels 1–6 ≈ CEFR A1–C2 (a rule of thumb, not official). */
    function topikOf(level) {
        var m = { 'A1': 1, 'A2': 2, 'B1': 3, 'B2': 4, 'C1': 5, 'C2': 6 };
        var parts = String(level).split(/[–-]/).map(function (p) { return m[p.trim()]; }).filter(Boolean);
        return parts.length ? ' (≈ TOPIK ' + parts.join('–') + ')' : '';
    }

    /* Summary points may carry <strong> emphasis; everything else stays escaped. */
    function rich(s) { return esc(s).replace(/&lt;(\/?)strong&gt;/g, '<$1strong>'); }
    ...
    function initObjectives() {
        ...
        sec.innerHTML =
            ...
            '<ul class="lx-check-list">' +
                info.objectives.map(function (o) { return '<li>' + rich(o) + '</li>'; }).join('') +
            '</ul>' +
            (info.time ? '<p class="lx-meta">⏱️ About ' + esc(info.time) + ' · ' +
                (info.level ? 'CEFR ' + esc(info.level) + topikOf(info.level) : 'Beginner') + '</p>' : '');

        // Place just below the lesson hero, above the first content section.
        var hero = document.querySelector('.hero');
        if (hero && hero.parentNode) { hero.parentNode.insertBefore(sec, hero.nextSibling); return; }
        // No hero (the beginner topic pages start with a bare <h1>): place the box
        // just BELOW the page title, never above it.
        var h1 = document.querySelector('body > h1') || document.querySelector('h1');
        if (h1 && h1.parentNode) { h1.parentNode.insertBefore(sec, h1.nextSibling); return; }
    ...
    function initSummary() {
        ...
        sec.innerHTML =
            ...
            '<ul class="lx-sum-list">' +
                info.summary.map(function (s) { return '<li>' + rich(s) + '</li>'; }).join('') +
            '</ul>';
    ...
    function initVocabReview() {
        ...
        function cardHTML(w) {
            var on = isTracked(w.ko);
            return '<div class="lx-word' + (on ? ' lx-known' : '') + (w.img ? ' lx-word-has-img' : '') + '" data-ko="' + esc(w.ko) + '">' +
                     vocabImg(w) +
                     '<div class="lx-word-tl"><span data-speak="' + esc(w.ko) + '">' + esc(w.ko) + '</span></div>' +
        ...
        function refresh() {
            var k = 0;
            list.forEach(function (w) { if (isTracked(w.ko)) k++; });
        ...
        sec.addEventListener('click', function (e) {
            ...
            var tl = card.getAttribute('data-ko');
            var on = !isTracked(tl);
            trackWord(tl, on);
        ...
        markAllBtn.addEventListener('click', function () {
            var allOn = list.every(function (w) { return isTracked(w.ko); });
            list.forEach(function (w) { trackWord(w.ko, !allOn); });
            Array.prototype.forEach.call(sec.querySelectorAll('.lx-word'), function (card) {
                var on = isTracked(card.getAttribute('data-ko'));
                card.classList.toggle('lx-known', on);
        ...
        function renderCard() {
            ...
            if (!flipped) {
                faceEl.innerHTML = (w.img ? '<div class="lx-flash-img">' + vocabImg(w) + '</div>' : '') +
                    '<div class="lx-flash-front"><span data-speak="' + esc(w.ko) + '">' + esc(w.ko) + '</span></div>' +
    ...
    /* ==========================================================
       Self-check quiz  (Korean → English, auto-generated)
       ========================================================== */
    function initQuiz() {
        var list = VOCAB[lessonNum];
        ...
        function build() {
            ...
            body.innerHTML = picks.map(function (ans, qi) {
                ...
                return '<div class="lx-quiz-q" data-qi="' + qi + '">' +
                         '<p class="lx-quiz-stem">' + (qi + 1) + '. What does <span class="lx-q-word" data-speak="' + esc(ans.ko) + '">' + esc(ans.ko) + '</span> mean?</p>' +
    ...
    function initJournal() {
        ...
        sec.innerHTML =
            ...
            '<div class="lx-journal-prompts">Reflection helps it stick. Try: <strong>What clicked?</strong> · ' +
                ...
                '<strong>A sentence I can now say in Korean.</strong></div>' +
            '<textarea id="lx-journal-text" class="lx-textarea" ' +
                'placeholder="Escribe aquí tus pensamientos… (Write your thoughts here.)"></textarea>' +
    ...
    function exportJournal() {
        ...
        function label(slug) {
            // slug is a page name: either a numbered intermediate page or a
            // topic-named beginner page. Resolve to "Lesson N — Title".
            var num = null;
            for (var s in SLUGS) { if (SLUGS.hasOwnProperty(s) && SLUGS[s] === slug) { num = s; break; } }
            if (!num) { var m = slug.match(/korean_lesson_(\d+)/); if (m) num = m[1]; }
            if (num) return 'Lesson ' + num + (TITLES[num] ? ' — ' + TITLES[num] : '');
            return slug;
        }
        var md = entries.length
            ? '# My Korean Learning Journal\n\n' + entries.map(function (e) {
                return '## ' + label(e.page) + '\n_saved ' + fmt(e.ts) + '_\n\n' + e.text + '\n';
              }).join('\n---\n\n')
        ...
        a.href = url; a.download = 'my-korean-journal.md';
    ...
    function initExercises() {
        /* Korean word spacing (띄어쓰기) is not what these drills test, so answers are
           compared with all whitespace removed; NFC folds decomposed jamo from some IMEs. */
        function norm(s) {
            return (s || '').normalize('NFC').trim().toLowerCase().replace(/[.!?,;:]+$/, '').replace(/\s+/g, '');
        }

        var boxes = document.querySelectorAll(
            '.practice-box ul, .practice-box ol, .practice-box .example-box, ' +
            '.practice-section ul, .practice-section ol, .practice-section .example-box');
        Array.prototype.forEach.call(boxes, function (box) {
            ...
            fields.forEach(function (f) {
                ...
                f.addEventListener('keydown', function (e) {
                    // A Korean IME uses Enter to commit the syllable being composed.
                    if (e.key === 'Enter' && !e.isComposing && e.keyCode !== 229) { e.preventDefault(); evaluate(); }
                });
            });
        });
    }
    ...
    function initDashboard() {
        ...
        dash.innerHTML =
            '<div class="lx-dash-card">' +
                ...
                '<div class="lx-toolbar">' +
                    '<a class="lx-btn lx-btn-primary" href="' + lessonHref(resume) + '">▶ ' + (visited ? 'Continue' : 'Start') + ': Lesson ' + resume + '</a>' +
        ...
        Array.prototype.forEach.call(container.querySelectorAll('.lesson-card'), function (card) {
            ...
            var cardSlug = (href.split('/').pop() || '').replace('.html', '');
            var num = null;
            for (var s in SLUGS) { if (SLUGS.hasOwnProperty(s) && SLUGS[s] === cardSlug) { num = s; break; } }
            if (!num) { var m = href.match(/korean_lesson_(\d+)/); if (m) num = m[1]; }
            if (!num) return;
            var st = p[num] || {};
