#!/usr/bin/env python3
"""Build an Anki .apkg study deck for the Korean INTERMEDIATE course (B1-B2).
...
Run:  python3 build_anki.py     (from korean/intermediate; needs `genanki`)
Output: korean_intermediate.apkg
"""
import json
import os
import re
...
def main():
    ...
    model = genanki.Model(
        MODEL_ID,
        "Korean Intermediate (Korean/English)",
        fields=[
            {"name": "Korean"},
            {"name": "Pronunciation"},
            {"name": "English"},
...
""",
    ...
    deck = genanki.Deck(DECK_ID, "Korean Intermediate (B1-B2 · TOPIK II 3-4)")
    ...
    out = os.path.join(HERE, "korean_intermediate.apkg")
