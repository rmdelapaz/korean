/* audio.js — Korean pronunciation via the Web Speech API.
   ...
   Attaches a speaker button to every table cell containing Korean text.
   Target text is matched by Unicode range, not column position: it sits in
   column 1 in most tables but not all, and no cell carries a lang attribute.

   If the device has no Korean voice installed the entire layer hides itself.
   An English voice reading Korean is worse than no audio at all. */
(function () {
    'use strict';
    ...
    var OVERRIDES = { 'ㄱ': '가', 'ㄴ': '나', 'ㄷ': '다', 'ㄹ': '라', 'ㅁ': '마', 'ㅂ': '바', 'ㅅ': '사', 'ㅇ': '아', 'ㅈ': '자', 'ㅊ': '차', 'ㅋ': '카', 'ㅌ': '타', 'ㅍ': '파', 'ㅎ': '하', 'ㄲ': '까', 'ㄸ': '따', 'ㅃ': '빠', 'ㅆ': '싸', 'ㅉ': '짜', 'ㅏ': '아', 'ㅑ': '야', 'ㅓ': '어', 'ㅕ': '여', 'ㅗ': '오', 'ㅛ': '요', 'ㅜ': '우', 'ㅠ': '유', 'ㅡ': '으', 'ㅣ': '이', 'ㅐ': '애', 'ㅒ': '얘', 'ㅔ': '에', 'ㅖ': '예', 'ㅘ': '와', 'ㅙ': '왜', 'ㅚ': '외', 'ㅝ': '워', 'ㅞ': '웨', 'ㅟ': '위', 'ㅢ': '의' };
